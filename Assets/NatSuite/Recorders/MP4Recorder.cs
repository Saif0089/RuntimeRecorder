namespace NatSuite.Recorders {
	...
	public sealed class MP4Recorder : NativeRecorder {
		...
		) : base(Create(filePath, width, height, frameRate, sampleRate, channelCount, videoBitRate, keyframeInterval, audioBitRate)) { }
	...
	int keyframeInterval = 2,
	int audioBitRate = 64_000
) : base(Create(Utility.GetPath(".mp4"), width, height, frameRate, sampleRate, channelCount, videoBitRate, keyframeInterval, audioBitRate)) { }
		#endregion
